...
\subsubsection{Masses of the $H$, \Hc, $A$ and $a$  bosons ($\mH$, $\mHc$, $\mA$ and $\ma$)}
...
\caption{\MET and  $\MT$ distributions after the full selection of Z(lep)+\MET search. Both distributions show a peaked structure around $\mH$ in the $\mH > \ma$ regime, reflecting the resonant production of $H$ with a subsequent decay $\pH\to\pa\pZ$.}
\label{fig:monoz_kin_final}
\end{figure}

In designing a search for evidence of this model, it may be useful to consider not only the $\MET$, but also the $\MT$\footnote{The massless definition is used here: $\MT = \sqrt{2 \met p_{\rm{T,Z}} \left(1-\cos\left(\Delta\phi\right)\right)}$} variable. The distributions of both variables after final selection are shown in ~\autoref{fig:monoz_kin_final} for the Z+\MET searches. Both distributions show Jacobian peak structures due to dominant effect of the diagram with resonant H exchange. For some parameter combinations, especially the most relevant case of $\ma<\mH$, the peak structure is more defined in the \MT distribution, thus helping to distinguish a possible signal from background. Where the resonant diagram does not contribute, i.e. for $\ma\approx\mH$ or $\ma>\mH$, the \MT distribution does not show a significantly different structure from the \MET distribution and will not provide an improved sensitivity.
...
In the $t\bar{t}$ + \MET signature, the $A$ ($h_{3}$ in the figure) and $a$  ($h_{4}$ in the figure) mass peaks are quite narrow for values where $\sin\theta$ approaches 1, and $a\rightarrow\chi\bar{\chi}$ is the dominant $\chi\bar{\chi}$ production mode, as shown in \autoref{fig:mchichi_sinp}. However, no significant kinematic dependence on $\sin\theta$ is observed in the $E_{T}^{miss}$ and top quark $p_{T}$ as shown in \autoref{fig:kin_sinp} before any analysis cuts are applied.
