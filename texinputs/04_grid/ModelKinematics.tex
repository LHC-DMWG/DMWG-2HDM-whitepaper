...
\subparagraph{\monohbb signature}
...
The exact event selection criteria are listed in the appendix, in \autoref{tab:monozll_selection}.
...
\subsubsection{Masses of the pseudoscalars $A$ and $a$ ($\mA$ and $\ma$)}
...
The pseudoscalar $A$ can be produced resonantly and decay in the $2\to1\to2$ process $gg\to A\to a h$ or $gg\to A\to a Z$, with the light pseudoscalar decaying invisibly as $a\to \chi\chi$. In this case, the $A \to a h $ process produces a resonance in the invariant mass distribution of the final state system with a  width determined by the widths of $a$, $A$, and of the SM bosons. This results in a peak in the transverse momentum distribution of the DM system, reconstructed as $\MET$ in the detector.
...
\begin{align}
E^{\mathrm{miss},max}_T \approx \frac{\sqrt{\left(\mA^2 -\ma^2 -M_{h/Z}^2\right)^2 - 4 \ma^2 M_{h/Z}^2}}{2\mA}\,.
\label{eq:monoH_peak_met}
\end{align}

Thus, increasing $\mA$ results in  a Jacobian peak at higher $\MET$, as shown in \autoref{fig:monoHbb_mA_scan_met} and \autoref{fig:monoz_ll_mA_scan}.
Conversely, models with higher $\ma$ have a Jacobian peak at lower $\MET$, as indicated in \autoref{fig:monoHbb_ma_scan_met}. 
For $\mA\approx\ma+m_{Z/H}$, both the $a$ and Z/H bosons are produced approximately at rest, leading to an event population with overall low boost. 
These qualitative trends are consistent with the distributions of the other main selection variables as shown in the appendix (\autoref{fig:monoz_kin_presel}).
...
\subsubsection{Mixing angle ($\tanb$)}
...
For \mDM (\autoref{fig:dm_scan_ll}, in the region where $\mDM < \frac{\ma}{2}$, DM mass has no effect on cross section or the kinematic distributions.  In the off-shell region where  $\mDM > \frac{\ma}{2}$ cross section steeply drops and the \MET distribution becomes less structured.
