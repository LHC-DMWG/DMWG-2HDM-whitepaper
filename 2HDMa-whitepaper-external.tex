...
The \hdma model is a 2HDM that contains, besides the Higgs doublets $H_1$ and $H_2$, an additional pseudoscalar singlet $P$. It is the simplest renormalisable extension of~\eqref{eq:simp} with an SM-singlet DM candidate~\cite{Ipek:2014gua,No:2015xqa,Goncalves:2016iyg,Bauer:2017ota,Tunney:2017yfp}. The gauge symmetry is made manifest by coudependenciespling the $P$ to the dark Dirac fermion  $\chi$ via
...
At present, dedicated ATLAS and CMS analyses of the $tW + \MET$  or other single-top-like signatures with $\MET$ do not exist. Performing such studies would however be worthwhile, since enhanced single-top signatures are expected to appear in many DM model that features an extended Higgs sector. 
